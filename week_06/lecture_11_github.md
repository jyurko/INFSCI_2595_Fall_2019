...
![](lecture_11_github_files/figure-gfm/viz_inverse_logit_func-1.png)<!-- -->
...
![](lecture_11_github_files/figure-gfm/show_eta_trends-1.png)<!-- -->
...
![](lecture_11_github_files/figure-gfm/show_mu_trends-1.png)<!-- -->
...
![](lecture_11_github_files/figure-gfm/show_eta_mu_together_f-1.png)<!-- -->
...
![](lecture_11_github_files/figure-gfm/viz_rand_bernoulli_draw_b-1.png)<!-- -->
...
![](lecture_11_github_files/figure-gfm/viz_rand_bernoulli_draw_c-1.png)<!-- -->
...
![](lecture_11_github_files/figure-gfm/viz_result_int_grid-1.png)<!-- -->
...
![](lecture_11_github_files/figure-gfm/viz_result_slope_grid-1.png)<!-- -->
...
With the Laplace approximation, the MVN mean is set equal to the
...
Inspecting the two expressions above reveals the only difference between
them is due the paritial derivative of the **linear predictor** with
respect to each parameter. The derivative of the log-likelihood with
respect to the probability,
![\\mu\_n](https://latex.codecogs.com/png.latex?%5Cmu_n "\\mu_n"), and
the derivative of the probability with respect to the linear predictor,
![\\eta\_n](https://latex.codecogs.com/png.latex?%5Ceta_n "\\eta_n"),
are the same\! Let’s now step through each derivative. The
![n](https://latex.codecogs.com/png.latex?n "n")-th log-likelihood’s
partial with respect to the event probability,
![\\mu\_n](https://latex.codecogs.com/png.latex?%5Cmu_n "\\mu_n"), of
the ![n](https://latex.codecogs.com/png.latex?n "n")-th observation is:
...
It can be shown that the derivative of the event probability,
![\\mu\_n](https://latex.codecogs.com/png.latex?%5Cmu_n "\\mu_n"), with
respect to the linear predictor,
![\\eta\_n](https://latex.codecogs.com/png.latex?%5Ceta_n "\\eta_n"), is
the **variance** of the Bernoulli random variable with event probability
...
" 
\\frac{\\partial L_n}{\\partial \\beta_0} = \\left( \\frac{y_n}{\\mu_n}-\\frac{1-y_n}{1-\\mu_n} \\right) \\left( \\mu_n \\left(1 - \\mu_n \\right) \\right) \\cdot 1 = y_n \\left(1 - \\mu_n\\right) - \\mu_n \\left(1 - y_n\\right)
")  
...
](https://latex.codecogs.com/png.latex?%20%0A%5Cfrac%7B%5Cpartial%20L_n%7D%7B%5Cpartial%20%5Cbeta_1%7D%20%3D%20%5Cleft%28%20%5Cfrac%7By_n%7D%7B%5Cmu_n%7D-%5Cfrac%7B1-y_n%7D%7B1-%5Cmu_n%7D%20%5Cright%29%20%5Cleft%28%20%5Cmu_n%20%5Cleft%281%20-%20%5Cmu_n%20%5Cright%29%20%5Cright%29%20%5Ccdot%20x_n%20%3D%20%5Cleft%28y_n%20%5Cleft%281%20-%20%5Cmu_n%5Cright%29%20-%20%5Cmu_n%20%5Cleft%281%20-%20y_n%5Cright%29%20%5Cright%29%20x_n%0A
" 
\\frac{\\partial L_n}{\\partial \\beta_1} = \\left( \\frac{y_n}{\\mu_n}-\\frac{1-y_n}{1-\\mu_n} \\right) \\left( \\mu_n \\left(1 - \\mu_n \\right) \\right) \\cdot x_n = \\left(y_n \\left(1 - \\mu_n\\right) - \\mu_n \\left(1 - y_n\\right) \\right) x_n
")  
...
event probability is the inverse logit-transformation of the linear
predictor, ![\\eta\_n](https://latex.codecogs.com/png.latex?%5Ceta_n
"\\eta_n"):
...
And finally, substituting in the general expression for the linear
predictor reveals that the derivative is a non-linear function of the
unknown parameters:
...
A closed form solution for the linear predictor parameters,
![\\boldsymbol{\\beta}](https://latex.codecogs.com/png.latex?%5Cboldsymbol%7B%5Cbeta%7D
"\\boldsymbol{\\beta}"), does not exist. We are therefore forced to
**numerically** determine the posterior mode.

Returning back to the expression for the derivative of the
![n](https://latex.codecogs.com/png.latex?n "n")-th likelihood, let’s
sum up all ![N](https://latex.codecogs.com/png.latex?N "N") individual
likelihoods to give:

  
![ 
\\frac{\\partial L}{\\partial \\beta\_d} = \\sum\_{n=1}^{N}
...
![ 
\\frac{\\partial L}{\\partial \\beta\_d} = \\mathbf{x}\_{:, d}^T \\left(
\\mathbf{y} - \\mathbf{\\mu} \\right)
...
![\\boldsymbol{\\beta}](https://latex.codecogs.com/png.latex?%5Cboldsymbol%7B%5Cbeta%7D
"\\boldsymbol{\\beta}")\! The difference between the linear model and
**logistic** model at this stage is due to the subtraction of the
observed outcome vector,
![\\mathbf{y}](https://latex.codecogs.com/png.latex?%5Cmathbf%7By%7D
"\\mathbf{y}"), by the unknown vector of observation event
probabilities,
...
L\_n}{\\partial \\beta\_d} \\right) = \\frac{\\partial}{\\partial
\\beta\_d} \\left( \\left(y\_n - \\mu\_n \\right) x\_{n,d} \\right) =
-x\_{n,d} \\frac{\\partial \\mu\_n}{\\partial \\beta\_d}
...
Expand the partial derivative of the linear predictor with respect to
![\\beta\_d](https://latex.codecogs.com/png.latex?%5Cbeta_d "\\beta_d")
with the chain rule:

  
![ 
\\frac{\\partial^2 L\_n}{\\partial \\beta\_d^2} = -x\_{n,d}
\\frac{\\partial \\mu\_n}{\\partial \\eta\_n} \\frac{\\partial
\\eta\_n}{\\partial \\beta\_d} = -x\_{n,d} \\mu\_n \\left(1 -
\\mu\_n\\right) x\_{n,d}
...
![N](https://latex.codecogs.com/png.latex?N "N") observations to yield
the derivatives associated with the *complete* log-likelihood,
![L](https://latex.codecogs.com/png.latex?L "L"). Before doing that
though, consider that the terms along the main diagonal will be summing
the squared inputs,
![\\left(x\_{n,d}\\right)^{2}](https://latex.codecogs.com/png.latex?%5Cleft%28x_%7Bn%2Cd%7D%5Cright%29%5E%7B2%7D
"\\left(x_{n,d}\\right)^{2}"). The off-diagonal terms will be summing up
the product of each input pair-wise combination, ![x\_{n,d} \\cdot
x\_{x,d\\prime}](https://latex.codecogs.com/png.latex?x_%7Bn%2Cd%7D%20%5Ccdot%20x_%7Bx%2Cd%5Cprime%7D
"x_{n,d} \\cdot x_{x,d\\prime}"). This is analogous to the **sum of
squares** matrix from the linear model\! In this context however, there
is an additional **weighting** factor equal to the variance of the
Bernoulli random variable, ![\\mu\_{n} \\left(1 -
\\mu\_{n}\\right)](https://latex.codecogs.com/png.latex?%5Cmu_%7Bn%7D%20%5Cleft%281%20-%20%5Cmu_%7Bn%7D%5Cright%29
"\\mu_{n} \\left(1 - \\mu_{n}\\right)"). The Hessian matrix can
...
We can simplify the Hessian matrix expression by defining a ![N \\times
...
"\\mathbf{S}"), which has zeros for all elements except those along the
main diagonal. The main diagonal consists of the Bernoulli variance
**weights**:
...
When we performed the Laplace approximation, `optim()` used an
...
matrix can be calculated analytically, assuming we know
...
like `optim()`) until convergence. The exact Hessian matrix is
calculated for the given parameter guess. Iterating like this yields the
new guess for the unknown parameters to be equal to the solution of a
**weighted** least squares problem. The weights are set by the
![\\mathbf{S}](https://latex.codecogs.com/png.latex?%5Cmathbf%7BS%7D
...
Let’s now see what happens if we were dealing with rare events. So,
perhaps the event, ![y=1](https://latex.codecogs.com/png.latex?y%3D1
...
are about 5x higher than in our synthetic data example\! As a reminder,
...
As shown below, the weights are the same as the previous example\!
**Why?**
...
The Bernoulli variance is the same whether
![\\mu](https://latex.codecogs.com/png.latex?%5Cmu "\\mu") equals 0.05
or 0.95\!

  
![ 
0.05 \\left(1 - 0.05\\right) = 0.05 \\cdot 0.95 
](https://latex.codecogs.com/png.latex?%20%0A0.05%20%5Cleft%281%20-%200.05%5Cright%29%20%3D%200.05%20%5Ccdot%200.95%20%0A
" 
0.05 \\left(1 - 0.05\\right) = 0.05 \\cdot 0.95 
")  
...
![ 
0.95 \\left(1 - 0.95\\right) = 0.95 \\cdot 0.05 
](https://latex.codecogs.com/png.latex?%20%0A0.95%20%5Cleft%281%20-%200.95%5Cright%29%20%3D%200.95%20%5Ccdot%200.05%20%0A
" 
0.95 \\left(1 - 0.95\\right) = 0.95 \\cdot 0.05 
")  

What if the event probability is even lower? For example, if the event
probability is 0.5%, ![\\mu\_n
= 0.005](https://latex.codecogs.com/png.latex?%5Cmu_n%20%3D%200.005
"\\mu_n = 0.005")? The Bernoulli variance is approximately equal to the
event probability\!

  
![ 
0.005 \\left(1 - 0.005 \\right)=0.005 \\cdot 0.995 \\approx 0.005
\\cdot 1 
](https://latex.codecogs.com/png.latex?%20%0A0.005%20%5Cleft%281%20-%200.005%20%5Cright%29%3D0.005%20%5Ccdot%200.995%20%5Capprox%200.005%20%5Ccdot%201%20%0A
" 
0.005 \\left(1 - 0.005 \\right)=0.005 \\cdot 0.995 \\approx 0.005 \\cdot 1 
")  

Calculate the weighted sum of squares matrix for the *very* rare event
case, and the resulting posterior covariance matrix. What happened?
...
The posterior variances are even larger\! What do you think this
represents in terms of the sample size to reduce the parameter posterior
uncertainty?
